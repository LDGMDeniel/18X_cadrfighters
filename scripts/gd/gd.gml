...
switch (argument0){
 ...
 case "chars": case "c": case 1:
		var gd_maxkey=array_length_2d(global.chars,argument1)-1;
		var gd_key=0;
		switch(argument2){
			...
			default:                          //или прямая ссылка на поле числом
				gd_key=-1; if(is_int32(argument2)){gd_key=argument2;}
				break; 
		}
		if(gd_key==0){
 ...
 case "item": case "i": case 2:
		var gd_maxkey=array_length_2d(global.item,argument1)-1;
		switch(argument2){	
			case "all": gd_key=0; break;      //выдать весь массив
			...
			case "iequip": gd_key=3 break;    //цвет - буква(r/b/y), цветовое ограничение экипировки
			...
			case "aresist": gd_key=10 break;  //сопротивление - буква(r/b/y), определяет уязвимость для оружия
			default:                          //или прямая ссылка на поле числом
				gd_key=-1; if(is_int32(argument2)){gd_key=argument2;}
				break; 
		}
		if(gd_key==0){
			var gd_t=array_create(gd_maxkey);
			for(var gd_i=1;gd_i<=gd_maxkey;gd_i+=1){
					gd_t[gd_i]=global.item[argument1,gd_i];
			}
			return gd_t; //возвращает всего персонажа в виде массива
		}else{
			if(gd_key==-1){
				...
				return global.item[argument1,gd_key]; //возвращает строку, букву, или число из ключа предмета
			}
		}
		break;
 ...
 //card/k/3 - gets kid(card but with "c" taken by chars) and key, returns card.key
	case "card": case "k": case 3:
		var gd_maxkey=array_length_2d(global.card,argument1)-1;
	 switch(argument2){	
			case "all": gd_key=0; break;      //выдать весь массив
		...
		if(gd_key==0){
			for(var gd_i=1;gd_i<=gd_maxkey;gd_i+=1){ //maxkey - максимальная длинна массива, не забывать менять
					gd_t[gd_i]=global.card[argument1,gd_i];
			}
			return gd_t; //возвращает всего персонажа в виде массива
		}else{
			if(gd_key==-1){
				...
				return global.card[argument1,gd_key]; //возвращает строку, букву, или число из ключа карты
			}
		}
	 break;
 ...
 //dcard - gets cid(chars) and key, returns returns character.device.card.key
	...
	case "ATBchars":
		var result=0;
	 switch(argument2){
			case "selected"://whatever is selected now
				var col=object0.ATB_s[0];
				var row=object0.ATB_s[1];
				result=object0.a_model[col,row].associated;
			 if(argument1!="id"){
					result=gd(1,result,argument1);
				}
				break;
			default://find by mypos
				var col=s[0];
				var row=s[1];
				result=object0.a_model[col,row].associated;
			 if(argument1!="id"){
					result=gd("c",result,argument1);
				}
				break;
		}
		return result;
	...
	case "ATBcard":
		...
		switch(argument2){
			case "selected":
				result=object0.a_card[object0.ATB_card].associated;
			 if(argument1!="id"){
					result=gd("k",result,argument1);
				}
				break;
		}
		return result;
